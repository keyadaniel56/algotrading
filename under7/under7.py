...
class FixedDigitHMMPredictor:
    ...
    def predict_under7_over2_proba(self, digit_sequence):
        """Predict probability of next digit being under 7 or over 2"""
        ...
        under7_prob = sum(digit_proba[digit] for digit in [0, 1, 2, 3, 4, 5, 6])
        over2_prob = sum(digit_proba[digit] for digit in [3, 4, 5, 6, 7, 8, 9])
       
        confidence = abs(under7_prob - over2_prob)
...
class FixedDerivTradingBot:
    def __init__(self, token=None, demo=True):
        ...
        # Martingale parameters from XML
        self.initial_stake = 0.35
        self.base_amount = self.initial_stake
        self.martingale_split = 2
        self.payout_percent = 39
        self.total_lost = 0.0
        self.count_loss = 0
    ...
    def trading_decision(self):
        ...
        prediction = self.hmm_predictor.predict_under7_over2_proba(self.digit_history[-50:]) # Longer recent sequence
        ...
        if prediction['under7'] > prediction['over2']:
            return 'DIGITUNDER', '7', f"UNDER7 (p={prediction['under7']:.3f}, conf={confidence:.3f})"
        else:
            return 'DIGITOVER', '2', f"OVER2 (p={prediction['over2']:.3f}, conf={confidence:.3f})"
    ...
    def analyze_prediction_patterns(self):
        ...
        under7_predictions = sum(1 for p in recent_predictions
                             if p['prediction']['under7'] > p['prediction']['over2'])
        under7_ratio = under7_predictions / len(recent_predictions)
       
        if under7_ratio > 0.85 or under7_ratio < 0.15: # Tighter bounds
            return f"Biased predictions ({under7_ratio:.1%} UNDER7) - consider retraining"
        else:
            return f"Normal patterns ({under7_ratio:.1%} UNDER7)"
    def calculate_trade_amount(self):
        ...
        amount = min(amount, risk_amount, max_risk_amount, self.max_amount)
        if amount < self.initial_stake:
            amount = self.initial_stake
        amount = round(amount, 2)
        self.log(f"Calculated amount: ${amount:.2f}")
        return amount
    def update_martingale(self, outcome, trade_amount, profit):
        """Update martingale based on trade outcome"""
        if outcome == 'loss':
            loss = -profit
        ...
        if self.total_lost > 0:
            ...
            if self.count_loss == 1:
                multiplier = 100 / self.payout_percent
                self.base_amount = round((self.total_lost * multiplier) / self.martingale_split, 2)
                self.log(f"Martingale updated: base_amount = ${self.base_amount:.2f}")
        else:
            self.count_loss = 0
            self.base_amount = self.initial_stake
            self.log("Martingale reset to initial stake")
    ...
    def place_trade(self, contract_type, barrier, symbol="R_100", amount=None):
        ...
        if amount <= 0:
            self.log("🛑 Amount <=0, pausing trade")
            return None
        ...
        try:
            ...
            if result:
                ...
                trade_record = {
                    ...
                    'status': 'placed',
                    'result': result,
                    'martingale_step': self.count_loss,
                    'contract_id': contract_id,
    ...
    def monitor_trade(self, contract_id, expected_direction):
        ...
        def trade_monitor():
            ...
            if contract_data:
                ...
                if self.trade_history:
                    ...
                    if 'profit' in contract_data:
                        ...
                        self.update_martingale(actual_outcome, last_trade['amount'], profit)
                ...
                if self.trade_history:
                    ...
                    self.current_profit += 0.0
                    self.update_martingale('loss', last_trade['amount'], 0.0)
            ...
            self.log(f"📊 Status - Count Loss: {self.count_loss}, Total Lost: ${self.total_lost:.2f}, Base Amount: ${self.base_amount:.2f}, Retrain pending: {self.pending_retrain}")
    ...
    def set_tp_sl(self, tp, sl):
        ...
        self.log(f"TP set to {self.take_profit}, SL set to {self.stop_loss}")
    def get_status(self):
        """Get current status"""
        with self.lock:
            status = {
    ...
    def get_chart_data(self):
        ...
        with self.lock:
            ...
            # Over 2 / Under 7 ratio
            over2_count = sum(1 for d in self.digit_history if d > 2)
            under7_count = sum(1 for d in self.digit_history if d < 7)
            ...
            data = {
                ...
                "over_under": [over2_count, under7_count],
        ...
        * {
        ...
        body::before {
            ...
            background: 
                ...
                radial-gradient(circle at 85% 25%, rgba(0, 255, 136, 0.06) 0%, transparent 40%),
        ...
        .header-left h1 {
            font-size: 32px;
            font-weight: 800;
            background: linear-gradient(90deg, var(--accent-white), var(--accent-orange), var(--accent-green));
...
<body>
    <div class="dashboard-container">
        ...
        <header class="dashboard-header">
            <div class="header-left">
                <h1>ALGOCDK</h1>
                <div class="subtitle">Advanced Algorithmic Trading System For Over 2 / Under 7</div>
            </div>
        ...
        <section class="controls-panel">
            ...
            <div class="tp-sl-controls">
                ...
                <input id="tp_input" type="number" value="100">
                <label>Stop Loss:</label>
                <input id="sl_input" type="number" value="50">
                <button class="btn btn-set-tpsl" onclick="set_tp_sl()">
                    <span>⚙</span>
                    SET TP/SL
                </button>
            </div>
                        ...
                        <tbody id="trades-body">
                            <tr>
                                <td colspan="5" style="text-align: center; padding: 60px 20px; color: var(--text-secondary);">
                                    <div style="opacity: 0.5; font-size: 14px; letter-spacing: 2px; text-transform: uppercase;">
        ...
        <div class="charts-grid">
            ...
            <div class="chart-container">
                <h2>Over 2 / Under 7 Pattern Ratio</h2>
    ...
    <script>
        ...
        // Only CSS has been enhanced
        let profitChart, digitsChart, digitDistChart, overUnderChart, confidenceChart;
        ...
        function initCharts() {
            ...
            // Over/Under Chart
            overUnderChart = new Chart(document.getElementById('overUnderChart'), {
                type: 'doughnut',
                data: {
                    labels: ['Over 2', 'Under 7'],
                    datasets: [{
                        data: [],
        ...
        function updateCharts(data) {
            ...
            // Over/Under
            if (data.over_under) {
                overUnderChart.data.datasets[0].data = data.over_under;
                overUnderChart.update('none');
            }
            ...
            // Update TP/SL inputs
            document.getElementById('tp_input').value = data.take_profit;
            document.getElementById('sl_input').value = data.stop_loss;
        }
        ...
        function set_tp_sl() {
            ...
            fetch(`/set_tp_sl?tp=${tp}&sl=${sl}`)
                .then(response => response.text())
                .then(data => {
                    addLog(`TP/SL set successfully`, 'success');
                    updateStatus();
                })
                .catch(error => {
                    addLog(`Error setting TP/SL: ${error.message}`, 'error');
                });
        }
...
class BotRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif parsed_path.path == '/set_tp_sl':
            params = parse_qs(parsed_path.query)
            tp = params.get('tp', [None])[0]
            sl = params.get('sl', [None])[0]
            if tp and sl:
                self.bot.set_tp_sl(tp, sl)
                response = "TP/SL set"
            else:
...
def main():
    ...
    if not bot.api_client.connect():
        ...
        return
    bot.api_client.ws_client.subscribe_ticks("R_100")
